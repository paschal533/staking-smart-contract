...
function Header() {
  const { signerAddress, fetchAccount } = useContext(StakerContext);
  ...
  return (
    <header className="sticky top-0 z-30 flex w-full items-center justify-between bg-[#E7ECEE] p-4">
      ...
      <div className="flex items-center justify-center gap-x-4 sm:w-[50vw] w-1/5">
        {!signerAddress ? <Button title="Connect Wallet" onClick={() => fetchAccount()} /> :  <Button title={`${signerAddress.slice(0,6)}...${signerAddress.slice(-5)}`} />}
      </div>
    </header>
  );
}
