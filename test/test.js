...
describe('Staking', function () {
    ...
    describe('stakeEther', function () {
        ...
        it('adds a positon to positions', async function() {
            ...
            const data = { value : transactionAmount }
            ...
            expect(position.weiStaked).to.equal(transactionAmount)
            expect(position.weiInterest).to.equal( ethers.BigNumber.from(transactionAmount).mul(1000).div(10000))
    ...
    describe('get position Ids for address', function() {
        it('returns a list of positionIds created by a specific address', async () => {
            ...
            const positionIds = await staking.getPositionIdForAddress(signer1.address)
